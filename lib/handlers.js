...
handlers._users.get = (data, callback) => {
  ...
  const phone = typeof(queryStringObject.phone) === 'string' && queryStringObject.phone.length === 11? queryStringObject.phone : false;
...
handlers._users.put = (data, callback) => {
  ...
  const phone = typeof(payload.phone) === 'string' && payload.phone.length === 11 ? payload.phone : false;
                    ...
                    _data.delete('checks', checkId, err => {
                      ...
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, { Error: 'Errors encountered while deleting user\'s checks' });
                        }
                      }
                    });
                  });
...
handlers._checks.put = (data, callback) => {
  ...
  // Error if the id is valid in all cases
  if (id) {
        ...
        if (!err && checkData) {
          ...
          handlers.tokens.verifyToken(token, checkData.userPhone, tokenIsValid => {
            if (tokenIsValid) {
                // Update the fields necessary
                if (protocol) {
                  checkData = { ...checkData, protocol };
                }
                if (url) {
                  checkData = { ...checkData, url };
                }
                if (method) {
                  checkData = { ...checkData,method };
                }
                if (successCodes) {
                  checkData = { ...checkData,successCodes };
                }
                if (timeoutSeconds) {
                  checkData = { ...checkData,timeoutSeconds };
                }
              ...
              if (!err) {
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if (!err && userData) {
                    ...
                    const checkPosition = userChecks.indexOf(checkData.id);
                    if (checkPosition > -1) {
                    ...
                    callback(500, { Error: 'Could not find user who created the check, so could not remove the check from the user object' });
                  }
                });
              } else {
